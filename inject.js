...
// We will inject the data as a JS script so it renders on page load
const scriptInjection = `
        ...
        function renderTab(index) {
            ...
            function buildList(inc, exc) {
                let html = "";
                inc.forEach(i => {
                    html += "<li class=\"flex items-start gap-3 text-sm text-gray-700\"><svg class=\"w-5 h-5 text-blue-500 mt-0.5 flex-shrink-0\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M5 13l4 4L19 7\"></path></svg><span>" + i + "</span></li>";
                });
                exc.forEach(e => {
                    html += "<li class=\"flex items-start gap-3 text-sm text-gray-400\"><svg class=\"w-5 h-5 text-blue-500 mt-0.5 flex-shrink-0\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M6 18L18 6M6 6l12 12\"></path></svg><span>" + e + "</span></li>";
                });
                return html;
            }
        ...
        function renderCustomPackages() {
            ...
            document.getElementById("custom-packages").innerHTML = html;
        }
`;

const containerRegex = /<div id="packages-container" class="w-full">[\s\S]*?<\/div>\s*<\/div>\s*<\/section>/;
html = html.replace(containerRegex, `<div id="packages-container" class="w-full">${packagesHTML}</div></div></section>`);

const sanityRegex = /\/\/ Fetch Mentoria Packages[\s\S]*?catch \(err\) \{[\s\S]*?\}/;
html = html.replace(sanityRegex, `// Mentoria Packages logic replaced with hardcoded data
        ${scriptInjection}
        renderTab(0);
